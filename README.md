...
订阅账号登录方式没有官方使用面板，本工具读取这些工具落在本地的会话日志做粗略统计（不追求 API 级精确）。统计默认只在本机处理；用户也可以主动登录 GitHub 并开启云同步，只同步 token、模型、来源、时间和请求数等统计字段。
...
- **可选云同步**：默认关闭；通过 GitHub 登录后可选择是否上传已有统计，并汇总多台电脑的 token 用量
...
- 云同步默认关闭，只上传标准化统计字段，不上传 prompt、回复、原始日志、项目路径或 AI 工具凭证。
...
- 云同步仅在用户登录并主动开启后工作；云端不保存 prompt、回复、原始日志或本地路径
- Tracker 云账号会话使用操作系统安全存储加密保存在本机
